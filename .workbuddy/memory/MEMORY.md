...
- **saver skill 修复清单**：#58 岗位校验失败→FATAL exit 1；#59 [D] 滚动改 CDP mouseWheel+scrollHeight 增长检测（scrollTop 赋值不触发懒加载）；#60 Send-Web curl `-m 20 --noproxy '*'` 硬超时；#61 选岗三级点击+验证闭环（26091609 晨批错岗位事故沉淀）；#62 停滞看门狗 exit 4 + wrapper 自动重启循环 + DownloadTarget 自动折算；#63 启动提速（wrapper 第1轮先探测、扩展活着跳过 daemon 重建；探测自适应间隔 0.5s→5s；run.ps1 导航/选岗/回顶固定睡改条件门兜底下的短等待）；#64 网页崩溃自愈（ok:false/空响应判崩溃 → Repair-PageCrash newTab 重建+岗位复核+回顶；面板阶段崩溃取消卡片登记重试；连续3次失败 exit 3 交 wrapper）；#65 wrapper 模板缺路径表读取→假 exit 0（"秒完成的成功"必须核查产物而非只看退出码）；#66 岗位标签小写+「·协作未上线」点号后缀→查找 JS 两侧 toLowerCase + Test-JobActive 增剥 `\s*·.*$` 与 -ieq。
- **多会话并行纪律（2026-09-16 撞号事故）**：并行会话可能同时改 SKILL.md——追加问题记录前必须先 Grep 最新编号再落笔；SKILL.md 物理重排用 ReadAllLines+WriteAllText(UTF8 BOM) 保编码。
- **26091609 ai产品经理 批次已完成全链路（2026-09-16 10:00）**：output/26091609 ai产品经理 下 100 份 docx（DONE 0 失败 0 跳过，09:02-09:36 约 34 分钟）+ ai产品经理.xlsx（100 提取→去重 12→88 行评估：高6/中53/低29，verify 通过）。高匹配：于女士33（B端+AI生图0-1）、宋女士31（数字化+AI大模型落地）、张先生29（RAG/LLM+Agent）、李先生34（以萨AI 5年+，疑似用户本人简历出现在推荐池）、杜女士29（AI管理平台+单据识别智能体）、阮先生27（智能码头AI产品线+售前）。
