...
| `output_xlsx` | string | **是** | 输出 Excel 文件绝对路径。**默认应使用模版格式输出（就地覆盖内容），模版优先取 skill 内置的 `templates/EvaluationTemplate.xlsx`**；用户明确要求旧 8 列格式时才自建新文件 |
...
**自 2026-09-15 起，评估结果默认按 `EvaluationTemplate.xlsx` 模版输出（覆盖模版内容），不再使用旧 8 列自建格式。** 模版已内置在本 skill 内：`templates/EvaluationTemplate.xlsx`（与本 SKILL.md 规则同步维护：12 列、无「建议行动」列、列宽 A/C=36 与 K/L=50 其余 12、全表垂直居中+自动换行、冻结首行）。默认直接使用该内置模版；用户另行提供模版路径时以用户的为准。

模版 12 列结构（Sheet1，首行表头，A2 冻结，斑马纹 F2F7FB，微软雅黑；⭐ 2026-09-15 起**无「建议行动」列**，重复简历不重复显示）。**列宽固定规则**：A 简历名称 / C 简要评价 = 36 字符，K 求职意向 / L 工作经历 = 50 字符，其余列 = 12 字符（`fill_excel_template` 已内置，覆盖模版原列宽）：

| 列 | 表头 | 来源 | 说明 |
|---|---|---|---|
| A | 简历名称 | 提取 | 文件名 |
| B | **胜任力评级** | AI | 高/中/低，配色：高=C6EFCE / 中=FFFFB1 / 低=FFD9D9（模版实测色，与旧版不同） |
| C | 简要评价 | AI | 标签总结，可附一句结论性建议（如"直接对口，优先约面""超龄且现居济南需沟通落地"），不用固定标签 |
| D | 年龄 | 提取 | 数字，居中 |
| E | 性别 | 提取 | 男/女 |
| F | **工作年限** | 提取 | 基本信息行第 3 段，如 `11年` |
| G | 现居住地 | 提取 | 城市 |
| H | **学历** | 提取 | 基本信息行第 4 段，如 `本科` |
| I | **求职状态** | 提取 | 基本信息行第 5 段，如 `离职-正在找工作` |
| J | **期望薪资** | 提取 | 从求职意向第一条自动提取，如 `8千-1.3万/月` |
| K | 求职意向 | 提取 | 格式：【岗位】薪资-地点-行业，多条换行 |
| L | 工作经历 | 提取 | 格式：【岗位】公司-时间，多条换行 |

其中加粗列（B/F/H/I/J）为相对旧版新增/更名的字段。基本信息行格式：`男｜38岁(1988年6月)｜11年｜本科｜离职-正在找工作`（性别｜年龄｜工作年限｜学历｜求职状态），由 `extract_docx.parse_basic_info` 解析。

**重复简历去重（⭐ 强制）**：写入前按「**姓名+年龄**」从文件名解析去重键（如 `吴先生_37岁_智联简历_21020.docx` → 键 `吴先生_37岁`），重复投递的简历**只保留首份**，不在结果中重复显示。`fill_excel_template()` 已内置该去重逻辑；AI 在评估前也须核对唯一行数，评估摘要中不得出现"重复投递简历"类条目。

**填充方式**：`fill_excel.fill_excel_template(json_path, template_xlsx)` 加载模版 → 保留表头 → 删除模版中的示例数据行 → 内置去重 → 写入新数据（样式自动采样自模版）。**所有单元格（含表头）一律垂直居中 + 自动换行（wrap_text=True）**——这是全表统一规则，无一例外，不许出现 vertical=top 或无换行的单元格；长文本列（A/C/K/L）靠换行 + 自适应行高展示，短文本列居中即可。B/C 列先留空，AI 评估后用 `write_evaluation_row(xlsx, row_index, rating, comment)` 按行索引写入（Excel 行 = JSON 索引 + 2）。

### 简要评价中的结论性建议（C 列，附属于标签）

C 列以标签总结为主，可在标签后补一句**结论性短句**，让 HR 一眼看出"为什么"。写法要求：

- 一句话（10~30 字），逗号分句，句末句号；先给核心判断，再补关键顾虑或亮点。
- 必须落到该候选人的具体差异点上：行业对口度、渠道经验、年龄、地域、薪资、稳定性、意向清晰度等，禁止千篇一律。
- 对口强 → 明确"优先约面/直接对口"；部分匹配 → 指出差距点+"可面谈/可培养"；不匹配 → 点明根因。

参考示例（仅示意风格，不要照抄、不要被局限）：
...
两种输出模式：
- `fill_excel(json_path, xlsx_path)`：旧 8 列格式（向后兼容）
- `fill_excel_template(json_path, xlsx_path)`：**默认推荐**，按 EvaluationTemplate.xlsx 模版 12 列填充（覆盖模版数据行，样式采样自模版；内置按「姓名+年龄」去重，重复简历只保留首份）
- `write_evaluation_row(xlsx_path, row_index, rating, comment)`：按行索引写入单条评估结果（B/C 列），row = index + 2

同时保留 `write_evaluations()`（仅适用于无重名的干净数据）。
...
- `extract_docx.py` 输出的 JSON 每项键为：`name` / `gender` / `age` / `current_residence` / **`work_years`** / **`education`** / **`job_status`** / `job_intentions` / `salary_info` / `work_experiences` / `work_descriptions` / **`filename`**。
- ⚠️ **字段名是 `filename`，不是 `file_name`**，读 JSON 时注意。
- `work_years`（工作年限）/ `education`（学历）/ `job_status`（求职状态）自基本信息行第 3/4/5 段解析（2026-09-15 新增），对应模版 G/I/J 列。
